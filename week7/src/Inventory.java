...
public class Inventory {
	...
	List<Item> items;
	int weekDay; // Saturday 1 through Sunday 7
	...
	private void setDiscounts() {
		for(Item item : items) {
			...
			// With gift card handled above, we don't need nearly as many if statements nested together
			if(weekDay == 4) { // day 4 is Tuesday
				item.price *= 0.9; // 10% discount on Tuesdays.
	...
	private void incrementDay() { // Resets things while moving day forward
		weekDay++; //increment to next day of week
		if(weekDay > 7) {
			weekDay = 1;
		}
		for(Item item: items) {
